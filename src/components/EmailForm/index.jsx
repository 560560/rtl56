...
const formAdditionalInfo = {
  home:
    'Наш специалист перезвонит Вам в ближайшее время для предоставления предварительного решения. ' +
    'Оставляя заявку, Вы даёте согласие на обработку персональных данных.',
  contacts:
    'Наш специалист перезвонит Вам в ближайшее время для предоставления ифнормации по интересующим Вас вопросам. ' +
...
const validationSchema = Yup.object({
  ...
  phone: Yup.string().required('Пожалуйста введите номер телефона').min(6, 'Номер не может быть короче 6 цифр'),
});
...
export const EmailForm = ({ sitePage, buttonTitle, additionalInfo }) => {
    ...
    return (
      <div className="emailFormWrapper">
        <div className="responseForm">
          ...
          <p>
            Если этого не произошло, Вы можете обратиться по номеру <b>8 3532 27-07-87</b>
          </p>
        </div>
      </div>
    );
            ...
            {({
              ...
              <Form autoComplete="off" noValidate onSubmit={handleSubmit}>
                ...
                <div className="button">
                  <button type="submit" disabled={isFetching} onClick={() => setValidationRequired(true)}>
                    <h4>{buttonTitle}</h4>
                  </button>
                </div>
